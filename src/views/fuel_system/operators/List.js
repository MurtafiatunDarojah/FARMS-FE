...
const ListOperator = () => {
    ...
    const getListOperator = () => {
        API.ListOperator(HeaderFarms(session.accessToken)).then(res => {
    ...
    const columns = [
        {
            ...
            renderCell: (data) => {
                return (
                    <>
                        <Button type='link' icon={<EditOutlined />} onClick={() => { navigate('/operator/' + data.row._id) }}></Button>
                    </>
                )
            }
        },
        ...
        {
            field: 'company_driver_license',
            headerName: 'ID Simper',
            width: 180
        },
        {
            field: 'company',
    ...
    return (
        <>
            <CCard>
                <CCardHeader>List Operator</CCardHeader>
                <CCardBody style={{ height: 50 }}>
                    <Button type='primary' icon={<UserAddOutlined />} onClick={() => navigate('/operator')}>Add Operator</Button>
                </CCardBody>
                <CCardBody>
                    <DataGrid
                        rows={listOperator}
